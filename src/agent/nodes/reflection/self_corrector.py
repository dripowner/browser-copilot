...
ОБЯЗАТЕЛЬНО ПОПРОБУЙ ЭТИ ПОДХОДЫ (в порядке приоритета):

1. **Прокрути страницу** — элемент может быть ниже:
   browser_scroll(direction="down", amount=500)

2. **Hover на контейнер** — кнопки часто скрыты до наведения:
   browser_hover(target="[родительский элемент]")
   Затем повтори исходное действие

3. **Другой тип селектора** — попробуй альтернативу:
   - Был CSS → попробуй role: "button:Добавить"
   - Был text → попробуй CSS: "[class*='add'], [class*='cart']"
   - Был сложный → упрости: только имя кнопки

4. **Исследуй заново:**
   browser_explore_page()
   Найди элемент с похожим назначением в результатах

5. **Клик на контейнер** — может открыть страницу с нужным элементом:
   browser_click(target="[карточка/контейнер]")

ЗАПРЕЩЕНО использовать browser_run_custom пока не попробованы минимум 3 подхода выше!"""
